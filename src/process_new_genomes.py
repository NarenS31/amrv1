"""
Process newly downloaded genomes into k-mer matrix
"""
import numpy as np
...
for i, genome_file in enumerate(genome_files):
    ...
    # Get assembly ID from filename
    assembly_id = genome_file.stem.replace('.fna', '')
    ...
    try:
        ...
        if len(sequence) < 100:  # Skip if too small
            continue
...
print(f"\n✓ Saved k-mer matrix: data/ncbi_missing/X_k6_new.npy")
print(f"✓ Saved genome IDs: data/ncbi_missing/genome_ids_new.txt")
